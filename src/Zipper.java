...
public class Zipper{
	...
	public void zipper(String DirectoryToZip, String outputFileName) throws IOException{
		...
		for (TreeNode node : nodesList) {
			if (node.myFile.isFile()) {
				...
				temp.createNewFile();
				huff.encode(node.myFile.toString(), temp.toString());
				TOC.append(node.myPathway + "," + byteStart);
				byteStart += temp.length();
	...
	public void unzipper(String filename, String place) throws IOException {
		tempPlace = place;
		compFileName = filename;
		unzipToMap(filename, place);
		createDirs(place);
		createTempFiles(place);
	}
	...
	public void createTempFiles(String place) throws IOException {
		...
		while (ptsVisited < indexes.size()) {
			...
			f.createNewFile();
			huffy.decode(temp.toString(), place + "/" + f.getPath());
			ptsVisited++;
			/*try {
	...
	/*try{
			 ...
			 System.err.println("A security violation has occured!!!!! File was not deleted properly");
		 }*/
